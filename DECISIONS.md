...
- *A validating admission webhook*: I'd implement a webhook to reject `UPDATE` request when immutable fields of the resource changed in YAML, with explicitly returning `HTTP 400` to user,
so `kubectl apply` returns a error instead of falling silently.
- *Synchronize state with kubernetes metav1.Condition protocol*: Instead of just simple `State: "PROVISIONING"` string I would implement proper standart Kubernetis conditions:
...
- *Obvservability*: I would add metrics about average provisioning time, count of `HTTP 500` responses, etc.
